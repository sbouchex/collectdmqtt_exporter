...
var (
	...
	collector     = &collectdmqttCollector{}
)
...
type ExporterMqttConfig struct {
	Topic  string `mapstructure:"topic" default:"/collectd/#"`
...
func metricHelp(group string, name string) string {
	return fmt.Sprintf("newcollectdmqtt exporter: Name: '%s_%s'", group, name)
}
...
type collectdmqttCollector struct {
	samples map[string]*newcollectdmqttSample
	mu      *sync.Mutex
	ch      chan *newcollectdmqttSample
}

func newcollectdmqttCollector() *collectdmqttCollector {
	c := &collectdmqttCollector{
		ch:      make(chan *newcollectdmqttSample, 0),
		mu:      &sync.Mutex{},
		samples: map[string]*newcollectdmqttSample{},
	}
	go c.processSamples()
...
func (c *collectdmqttCollector) processSamples() {
...
func (c collectdmqttCollector) Collect(ch chan<- prometheus.Metric) {
	ch <- lastPush

	c.mu.Lock()
	samples := make([]*newcollectdmqttSample, 0, len(c.samples))
...
func (c collectdmqttCollector) Describe(ch chan<- *prometheus.Desc) {
	ch <- lastPush.Desc()
}
...
func startExporter() {
	...
	collector = newcollectdmqttCollector()
	...
	opts.SetClientID("collectdmqtt_exporter_client")
...
func LoadConfig(path string) (err error) {
	...
	viper.SetConfigName("collectdmqtt_exporter")
...
func main() {
	viper.SetEnvPrefix("COLLECTDMQTT_EXPORTER")
